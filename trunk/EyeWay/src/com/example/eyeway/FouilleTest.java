...
public class FouilleTest extends Activity {
    ...
    public void testgetLieuParProximite(){
    	...
    	ListeLieu Lieux = fd.getLieuProximiteParType(47.845489, 1.939776,types, 10);
		/*
    	for(Lieu l : Lieux) {
			l.toString();
		}
		*/
    	AlertDialog alertDialog = new AlertDialog.Builder(this).create();
        alertDialog.setTitle("Resultat de la requete...");
        alertDialog.setMessage(Lieux.results.get(0).toString());
        alertDialog.setButton("OK", new DialogInterface.OnClickListener() {
           public void onClick(DialogInterface dialog, int which) {
              // here you can add functions
           }
    ...
    public void testgetLieuParRecherche(){
    	FouilleDonnee fd=new FouilleDonnee();
    	ListeLieu Lieux = fd.getLieuParRecherche("restaurant olivet");
		/*
    	for(Lieu l : Lieux) {
			l.toString();
		}
